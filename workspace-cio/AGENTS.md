...
| klaire | Klaire-投资管理 | 持仓表 | 报告表 | 交易记录 | 观察池 | 监控记录+KPI追踪+执行手册+说明 | 群（见 CONFIG_KLAIRE.md） |
...
- `群聊（Klaire+投委会+Towney）`是两个 principal 共用的渠道，回复前必须先确认这条消息/这次操作归属哪个 principal
  （谁在说话、提到了哪个 principal 的标的/账户），再用该 principal 自己的 `app_token`/表名去读写——绝不能因为
  "反正在同一个群"就把两人的持仓、建议、Bitable 记录混在一起展示或写错表。
- 单聊（Towney）天然只有一个 principal，没有串号风险，正常全权操作。
