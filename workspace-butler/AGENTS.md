...
消息含 cycle_id：先读 `~/.openclaw/shared/cycles/{cycle_id}/ic_request.json`，从中获取 `flow_type`、`principal`、`symbol`，再写 context.json。
...
```
~/.openclaw/shared/corona-status/{principal}.json
```
