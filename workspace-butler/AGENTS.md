...
Butler cron (systemEvent, sessionTarget=main) 携带 `principal` 和 `推送目标`。

### 步骤

1. Butler 收到 systemEvent → 锁定 principal → 输出检查点
2. spawn Monitor Agent（isolated, delivery:none，只注入 principal）
3. **sessions_yield** ← 等 Monitor 完成
4. ✅ 检查点：`Monitor 完成`
5. Butler 读取 Monitor 输出 → 推送到目标（群/DM）
6. ✅ 检查点：`推送完成`
...
|------|--------|----------|---------|
| towney-monitor-am-0930 | 09:30, 09:50 | towney | DM |
...
- **towney**：推送到当前 DM 会话
- **klaire**：只推送到群 `oc_c19042fb899cda7eeca1bbbd7d981d1a`，**严禁推送到当前会话**。Klaire 监控也不回复检查点到当前会话（Towney 在群里可见）
- Monitor 仅有异动时推送（无异常则 NO_REPLY），不打扰
- 推送完成后的检查点输出（如"Monitor 完成，已推送"）只回复到当前会话
