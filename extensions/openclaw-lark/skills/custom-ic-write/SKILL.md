...
2. **交易记录表**（决议含建仓/减仓/止损/清仓指令时）：symbol、action、position_pct、price_ref、operator="CIO"、status="建议/待执行"
   > ⚠️ 交易记录此时记录的是 **CIO 建议**，不代表用户已执行。用户确认执行后由该 principal 的前端 agent（klaire→Butler，towney→Dexter）将 status 更新为"已执行"，并同步更新持仓表。
   >
   > **写完交易记录后，立即更新持仓表对应行的 `IC最后更新日期` 字段（见下方步骤）。**
...
**触发条件：** 决议包含明确 symbol 时**无论 cio_action 类型**（含持有观察）均执行。

**目的：** Monitor Check D 依赖此字段判断最近一次 IC 审查时间。「持有观察」同样是一次完整 IC 审查，不更新会导致 Monitor 误报「未经IC审查」。

**步骤（时机：有交易指令时在写交易记录后立即执行；cio_action=持有观察无交易记录时，在写决策复盘表后立即执行）：**

1. 用 context.json 中的 `positions_table_id` 列出持仓表字段，确认 `IC最后更新日期` 字段是否存在
2. **若不存在**：跳过，在完成输出中注明 `（持仓表未设 IC最后更新日期 字段，跳过更新；如需 Monitor 陈旧检查，请在 Bitable 持仓表中添加该日期字段）`
3. **若存在**：
   - `feishu_bitable_app_table_record.list(positions_table_id, filter=股票代码={symbol} AND 状态=持有)` → 取 record_id
   - `feishu_bitable_app_table_record.update(record_id, {IC最后更新日期: 今日日期})`
   - 多个 symbol 时逐一更新
