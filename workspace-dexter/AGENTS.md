...
用户到月末说"算X月收益率"时，Dexter 直接计算：

**公式**：
```
X月总收益 = 已实现部分（当月所有卖出/减仓的贡献加总，已在执行确认时算好）
          + 浮动部分（月末持仓标的按月末收盘价折算）

浮动部分 = Σ [(月末收盘价 / 基准价 - 1) × 持有仓位%]  对每个仍持有的标的
```

**数据来源**：
- 基准价：`📅 月度基准快照` 表 `tblFU0KPryDKVfDK`
- 当月所有执行确认记录：从聊天记录或 Bitable 交易记录表提取
- 月末收盘价：`custom-market-data-cn` / `custom-market-data-us` SKILL 查询
- 持仓仓位：Bitable 持仓表 `占总仓位比例` 字段
...
⚠️ 此计算由 Dexter 自己执行，不需要 spawn 子 Agent。
